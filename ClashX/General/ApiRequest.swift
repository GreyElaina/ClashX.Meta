...
class ApiRequest {
    ...
    static func requestConfigUpdate(
        configName: String, callback: @escaping ((ErrorString?) -> Void)
    ) {
    ...
    static func requestConfigUpdate(
        configPath: String, callback: @escaping ((ErrorString?) -> Void)
    ) {
        let placeHolderErrorDesp = "Error occoured, Please try to fix it by restarting ClashX. "
        req(
            "/configs", method: .put, parameters: ["Path": configPath],
    ...
    static func updateOutBoundMode(mode: ClashProxyMode, callback: ((Bool) -> Void)? = nil) {
        req(
            "/configs", method: .patch, parameters: ["mode": mode.rawValue],
            encoding: JSONEncoding.default
        )
    ...
    static func updateLogLevel(level: ClashLogLevel, callback: ((Bool) -> Void)? = nil) {
        req(
            "/configs", method: .patch, parameters: ["log-level": level.rawValue],
            encoding: JSONEncoding.default
    ...
    static func requestProxyProviderList(completeHandler: ((ClashProviderResp) -> Void)? = nil) {
        req("/providers/proxies")
            .responseDecodable(of: ClashProviderResp.self, decoder: ClashProviderResp.decoder) {
                resp in
                switch resp.result {
...
extension ApiRequest {
    static func getConnections(
        completeHandler: @escaping ([ClashConnectionBaseSnapShot.Connection]) -> Void
    ) {
    ...
    static func getConnections(completeHandler: @escaping (DBConnectionSnapShot) -> Void) {
        ...
        req("/connections").responseDecodable(of: DBConnectionSnapShot.self, decoder: decoder) {
            resp in
            switch resp.result {
            case let .success(snapshot):
...
extension ApiRequest {
    ...
    static func updateProvider(
        for type: ProviderType, name: String, completeHandler: ((Bool) -> Void)? = nil
    ) {
    ...
    {
        req("/providers/rules")
            .responseDecodable(of: ClashRuleProviderResp.self, decoder: ClashProviderResp.decoder) {
                resp in
                switch resp.result {
                case let .success(providerResp):
                    completeHandler(providerResp)
    ...
    static func flushDNSCache() {
        ...
        group.notify(queue: .main) {
            ...
            UserNotificationCenter.shared.post(
                title: NSLocalizedString("Flush dns cache", comment: ""), info: info)
        }
    }
    ...
    static func updateProvider(
        name: String, type: ProviderType, completeHandler: @escaping (Bool) -> Void
    ) {
        let url: String
        switch type {
        case .proxy:
...
extension ApiRequest: WebSocketDelegate {
    ...
    func websocketDidDisconnect(socket: WebSocketClient, error: Error?) {
        ...
        case memoryWebSocket:
            ...
            memoryWebSocketRetryTimer =
                Timer.scheduledTimer(
                    withTimeInterval: memoryWebSocketRetryDelay, repeats: false,
                    block: {
                        [weak self] _ in
    ...
    func websocketDidReceiveMessage(socket: WebSocketClient, text: String) {
        ...
        case trafficWebSocket:
            ...
            dashboardDelegate?.didUpdateTraffic(
                up: json["up"].intValue, down: json["down"].intValue)
        case loggingWebSocket:
            Task {
                guard await logRateLimiter.processLog() else { return }
                delegate?.didGetLog(
                    log: json["payload"].stringValue, level: json["type"].string ?? "info")
                dashboardDelegate?.didGetLog(
                    log: json["payload"].stringValue, level: json["type"].string ?? "info")
            }
        case memoryWebSocket:
